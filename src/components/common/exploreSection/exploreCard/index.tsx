...
const ExploreCard:React.FC<IProps> = ({restaurant}) => {
    ...
    const approxPrice = restaurant?.info?.cfo?.text;
        ...
        const cuisines = restaurant?.info?.cuisine?.map((item:any)=>item.name).slice(0, 3);
    ...
    <div className='explore-card cur-po'>
        ...
        <div className="res-row">
            {cuisines.length  && (<div className='res-cuisines'>
                {cuisines.map((item:any, i:number)=>{
                    return(
        ...
        {/* bottom container */}
        {bottomContainers.length > 0 && <div className='card-seperator'>
